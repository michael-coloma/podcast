export const convertFormatZuluToFormatDateView = (formatZulu: string): string => {
  ...
  const formatDate = (d: Date, separator = " "): string | null => {
    ...
    const n = [d.getDate(), d.getMonth() + 1, d.getFullYear(), d.getHours(), d.getMinutes()];

    return `${pad(n[0])}/${pad(n[1])}/${n[2]}${separator}${pad(n[3])}:${pad(n[4])}`;
  };

  const formatZonedTimestamp = (date: string | undefined | null, separator = " "): string | null => {
    if (!date) {
      return null;
    }
